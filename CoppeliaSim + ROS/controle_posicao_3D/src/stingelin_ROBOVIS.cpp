...
bool efeito_derivativoNAN1 = false;
	...
	efeito_derivativoNAN1 = isnan(efeito_derivativo1) ? true : false;
		...
		if(efeito_derivativoNAN1){
			...
			efeito_derivativo1 = efeito_derivativo1-40;
			return efeito_derivativo1;
		}
...
bool efeito_derivativoNAN2 = false;
	...
	efeito_derivativoNAN2 = isnan(efeito_derivativo2) ? true : false;
		...
		if(efeito_derivativoNAN2){
			...
			efeito_derivativo2 = efeito_derivativo2-40;
			return efeito_derivativo2;
		...
		while ((abs(error_dist_robo) > tolerance_pos) && (abs(_error_dist_robo) > _tolerance_pos)) {
			...
			if (_is_obs){
				...
				if(_dcml_coordY > 2 && _dcml_coordY < 200){
					...
					_msg.linear.x = -4 + (ajuste1(efeito_derivativo)*0.02);
					...
					printf("velocidade KP azul (REAL): %.2f m\n",_msg.linear.x);
						...
						_msg.linear.x = -4 + (ajuste1(efeito_derivativo)*0.01);
				...
				if (_dcml_coordY < -2 && _dcml_coordY > -200){
					...
					msg.linear.x = -4 + (ajuste2(efeito_derivativo)*0.02);
					...
					loop_rate.sleep();
						...
						msg.linear.x = -4 + (ajuste2(efeito_derivativo)*0.01);
